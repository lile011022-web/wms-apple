...
- Normal import mode rejects existing SKU or UPC conflicts before any product row is created.
- Operators can explicitly enable `按 SKU 覆盖更新已有商品`. In this mode, an existing SKU is
  updated and a new SKU is created in the same all-or-nothing transaction.
- Overwrite import replaces the existing product profile and UPC list with the spreadsheet row while
  preserving the product's active/inactive status. Blank optional spreadsheet cells clear those fields.
- An imported UPC may remain attached to the same existing SKU, but a UPC owned by another SKU rejects
  the complete import. Import never transfers UPC ownership between products.
...
- The first worksheet is `可重新导入`. It uses the exact import-template headers and `true` / `false`
  values for `requiresImei`, so the exported workbook can be selected directly and re-imported with the
