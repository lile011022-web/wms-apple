...
- `预报`：WMS 写入和更新，对方系统读取。
- `订单`：已有订单主数据，WMS 读取，用 Apple 订单链接补全真实物流单号、姓名和仓库。
- `状态`：对方系统写入，WMS 读取。

其他 sheet 不允许读写，避免误改对方现有数据。

`预报`、`订单` 和 `状态` 第一行必须保留为字段表头，字段名应与下方模板一致。

表结构：

| Sheet | Writer        | Reader   | Purpose                                |
| ----- | ------------- | -------- | -------------------------------------- |
| 预报  | WMS           | 对方系统 | WMS 每天或手动把新增预报推给对方读取；订单补全后更新原行。 |
| 订单  | 对方/人工已有数据 | WMS | WMS 按 Apple 链接查真实物流单号、仓库和姓名。 |
...
| ------------ | -------- | ------------------------------------------- |
| 链接         | No       | Apple 订单链接。                            |
| 型号         | No       | 导入模板或订单补全提供的型号。              |
| 姓名         | No       | 导入模板或订单补全提供的收件/订单姓名。     |
...
| Field    | Required | Description                                      |
| -------- | -------- | ------------------------------------------------ |
| 预报仓库 | No       | WMS 读取后写入本地预报备注，并回写到 `预报.仓库`。 |
| 单号 / 物流单号 | No | 真实承运商物流单号，用于替换本地 `APPLE-W...`。 |
| 邮箱     | No       | 订单上下文，当前不入库保存。                     |
| 名字     | No       | WMS 读取后写入本地 `recipientName`。              |
| 超链接   | Yes      | Apple 订单链接，用于匹配 WMS 本地预报。           |
...
1. WMS 首次把本地预报写到 `预报`。
2. WMS 读取 `订单`，优先按 `超链接` 中的 Apple 订单号匹配本地预报。
3. `订单.单号` 或 `订单.物流单号` 有真实物流单号时，WMS 替换本地 `APPLE-W...` 订单引用，并识别承运商。
4. `订单.预报仓库` 和 `订单.名字` 回写本地预报，并更新 `预报` 原行，不新增重复行。
5. WMS 读取 `状态` 时，优先用 `预报ID` 匹配，其次用标准化后的 `物流单号` 匹配。
6. `状态` 中 `预报ID` 和 `物流单号` 都为空时，用 `链接`、`订单链接` 或 `Apple订单链接` 匹配原始 Apple 订单链接或 Apple 订单号。
7. `入库日期` 或 `入库时间` 有值时，WMS 标记为已收到。
8. `入库状态` 为 `已收到`、`已入库` 或 `RECEIVED` 时，WMS 标记为已收到。
9. `提醒` 包含 `未收到` 时，WMS 生成送达未入库异常。
10. `订单状态 = DELIVERED` 且 `入库日期` 为空时，WMS 生成送达未入库异常。
11. 同一个物流单号匹配多个客户时，不自动确认，进入冲突处理。
