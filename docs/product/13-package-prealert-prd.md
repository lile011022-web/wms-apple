...
| Receiving Status       | WMS 内部状态，如未入库、部分入库、已入库、已作废。                                          |
| Alert                  | 系统发现风险后生成的提醒，如送达未入库。                                                    |
...
| ----- | ------------- | -------- | -------------------------------------- |
| 预报  | WMS           | 对方系统 | WMS 每天或手动把新增预报推给对方读取。 |
| 状态  | 对方系统/仓库 | WMS      | 对方回写扫码收货、未收到和异常结果。   |

`预报` sheet 第一版字段：

| Field        | Required | Description                                 |
| ------------ | -------- | ------------------------------------------- |
| 预报ID       | Yes      | WMS 预报明细 ID，回传时优先用它匹配。       |
| 客户         | Yes      | 客户名称。                                  |
| 客户编码     | No       | WMS 客户编码。                              |
| 订单链接     | No       | Apple 订单链接或原始来源链接。              |
| Apple订单号  | No       | 从 Apple 链接提取的 W 开头订单号。          |
| 物流类型     | No       | UPS、USPS、FedEx、Other、Unknown。          |
| 物流单号     | Yes      | 双方兜底匹配主键。                          |
| 物流查询链接 | No       | 承运商查询链接。                            |
| 预计交付日期 | No       | 物流系统返回的 ETA 或原始预计交付文本。     |
| 送达日期     | No       | 物流显示 delivered 的日期。                 |
| 仓库         | No       | 目标仓库或对方仓库代码。                    |
| 订单状态     | Yes      | 当前物流状态，如 `UNKNOWN` 或 `DELIVERED`。 |
| 入库日期     | No       | WMS 推送时为空，回传表负责写入。            |
| 提醒         | No       | WMS 推送时为空，回传表负责写入异常提醒。    |
| 推送状态     | Yes      | `NEW`、`READ` 或对方约定状态。              |
| 推送时间     | Yes      | WMS 写入 Google Sheet 的时间。              |
| 仓库         | No       | 人工录入或 Excel 模板中的预报仓库代码。     |
...
1. WMS 优先用 `预报ID` 匹配预报包裹。
2. `预报ID` 为空时，用标准化后的 `物流单号` 匹配。
3. `预报ID` 和 `物流单号` 都为空时，用 `链接`、`订单链接` 或 `Apple订单链接` 匹配原始 Apple 订单链接或 Apple 订单号。
4. 如果本地当前是 `APPLE-W...` 订单引用，而回传行带来了真实承运商 `物流单号`，WMS 把本地单号替换为真实物流单号，并根据 `物流类型` 或 `物流查询链接` 识别承运商。
5. 如果该预报尚未入库，WMS 把它重新标记为待推送；下一次写入 `预报` sheet 时，会追加一行带真实物流单号、物流类型和物流查询链接的数据。
