...
3. 已完成 `apps/api` 的 NestJS 基础框架、数据库核心模型、认证、用户、角色、权限、仓库、系统设置、客户管理、UPC 商品库、入库扫码、入库记录、客户库存、出库装箱、异常池、批量修改客户和报表导出模块。
4. 下一步继续实现 Dashboard 和审计日志查询后端流程。
5. 再接入 `apps/web` 的 React 页面与真实后端 API。
6. 最后补齐 Dashboard、审计日志查询和前端真实接口接入。
